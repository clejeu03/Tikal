	...
	public class CameraMediator : Mediator {
		...
		void OnGUI() {
			if (initialSequence) {
				...
				if (characterAttach) {
					GUI.Label(new Rect(20, y, 200, 40), "State: KEYBOARD attached...");
				}
			} else {
				...
				btnStyle.normal.background = (model.state == CameraState.KEYBOARD ? btnActive : btnInactive);
				if (GUI.Button(new Rect(185, 40, 80, 20), "Character", btnStyle)) {
					model.SetState(CameraState.KEYBOARD);
					view.stateChange(CameraState.KEYBOARD);
				}
				...
				} else if (model.state == CameraState.KEYBOARD) {
